        ...
        {loading ? (
          ...
          <div className="bg-white shadow-lg rounded-lg p-4 sm:p-6 lg:p-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start gap-3 mb-4">
              <div className="flex-1">
                ...
                <p className="text-xs sm:text-sm text-gray-600">Invoice No: <span className="font-semibold">{invoiceData.invoiceNo}</span></p>
                <p className="text-xs sm:text-sm text-gray-600">Date: <span className="font-semibold">{new Date(invoiceData.date).toLocaleDateString('en-IN')}</span></p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
            ...
            <div className="border border-gray-300 rounded-lg p-3 sm:p-4">
              ...
              <h4 className="text-base sm:text-lg font-bold text-gray-900 mb-2">{invoiceData.seller.name}</h4>
              {invoiceData.seller.addressLine1 && <p className="text-xs sm:text-sm text-gray-700 leading-relaxed">{invoiceData.seller.addressLine1}</p>}
              {invoiceData.seller.addressLine2 && <p className="text-xs sm:text-sm text-gray-700 leading-relaxed">{invoiceData.seller.addressLine2}</p>}
              {invoiceData.seller.city && <p className="text-xs sm:text-sm text-gray-700 leading-relaxed">{invoiceData.seller.city}, {invoiceData.seller.state}</p>}
              {invoiceData.seller.gstin && <p className="text-xs sm:text-sm text-gray-700 mt-2"><span className="font-semibold">GSTIN:</span> {invoiceData.seller.gstin}</p>}
              {invoiceData.seller.phone && <p className="text-xs sm:text-sm text-gray-700"><span className="font-semibold">Phone:</span> {invoiceData.seller.phone}</p>}
              
              {/* Bank Details */}
              {invoiceData.seller.bankName && (
                <div className="mt-3 pt-3 border-t border-gray-200">
                  <p className="text-xs font-semibold text-gray-600 mb-1">Bank Details:</p>
                  <p className="text-xs sm:text-sm text-gray-700">{invoiceData.seller.bankName}</p>
                  {invoiceData.seller.bankAccount && <p className="text-xs sm:text-sm text-gray-700">A/c: {invoiceData.seller.bankAccount}</p>}
                  {invoiceData.seller.ifsc && <p className="text-xs sm:text-sm text-gray-700">IFSC: {invoiceData.seller.ifsc}</p>}
                </div>
              )}
            </div>
            ...
            <div className="border border-gray-300 rounded-lg p-3 sm:p-4">
              ...
              <h4 className="text-base sm:text-lg font-bold text-gray-900 mb-2">{invoiceData.buyer.name}</h4>
              {invoiceData.buyer.addressLine1 && <p className="text-xs sm:text-sm text-gray-700 leading-relaxed">{invoiceData.buyer.addressLine1}</p>}
              {invoiceData.buyer.addressLine2 && <p className="text-xs sm:text-sm text-gray-700 leading-relaxed">{invoiceData.buyer.addressLine2}</p>}
              {invoiceData.buyer.city && <p className="text-xs sm:text-sm text-gray-700 leading-relaxed">{invoiceData.buyer.city}, {invoiceData.buyer.state}</p>}
              {invoiceData.buyer.gstin && <p className="text-xs sm:text-sm text-gray-700 mt-2"><span className="font-semibold">GSTIN:</span> {invoiceData.buyer.gstin}</p>}
              {invoiceData.buyer.phone && <p className="text-xs sm:text-sm text-gray-700"><span className="font-semibold">Phone:</span> {invoiceData.buyer.phone}</p>}
            </div>
          </div>
            ...
            <div className="w-full sm:w-auto sm:min-w-[280px] lg:min-w-[320px]">
              <div className="space-y-2">
                <div className="flex justify-between items-center pb-2">
                  ...
                  <span className="text-xs sm:text-sm font-semibold text-gray-900">{formatIndianCurrency(invoiceData.subtotal)}</span>
                </div>
                ...
                {invoiceData.seller.stateCode?.trim() && invoiceData.buyer.stateCode?.trim() && invoiceData.seller.stateCode?.trim() !== invoiceData.buyer.stateCode?.trim() ? (
                  <div className="flex justify-between items-center pb-2">
                    <span className="text-xs sm:text-sm text-gray-700">IGST @ {(parseFloat(invoiceData.sgstRate) + parseFloat(invoiceData.cgstRate)).toFixed(2)}%:</span>
                    <span className="text-xs sm:text-sm font-semibold text-gray-900">{formatIndianCurrency((parseFloat(invoiceData.sgstAmount) + parseFloat(invoiceData.cgstAmount)).toFixed(2))}</span>
                  </div>
                ) : (
                  <>
                    <div className="flex justify-between items-center">
                      <span className="text-xs sm:text-sm text-gray-700">SGST @ {invoiceData.sgstRate}%:</span>
                      <span className="text-xs sm:text-sm font-semibold text-gray-900">{formatIndianCurrency(invoiceData.sgstAmount)}</span>
                    </div>
                    
                    <div className="flex justify-between items-center pb-2">
                      <span className="text-xs sm:text-sm text-gray-700">CGST @ {invoiceData.cgstRate}%:</span>
                      <span className="text-xs sm:text-sm font-semibold text-gray-900">{formatIndianCurrency(invoiceData.cgstAmount)}</span>
                    </div>
                  </>
                )}
                ...
                <div className="flex justify-between items-center pt-2 border-t border-gray-300">
                  ...
                  <span className="text-xs sm:text-sm font-semibold text-gray-900">{formatIndianCurrency(invoiceData.totalTax)}</span>
                </div>
                ...
                <div className="flex justify-between items-center pt-3 border-t-2 border-gray-900 mt-3">
                  ...
                  <span className="text-lg sm:text-xl font-bold text-blue-600">{formatIndianCurrency(invoiceData.totalAmount)}</span>
                </div>
              </div>
            ...
            <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-gray-300">
              {invoiceData.deliveryNote && (
                <div className="mb-4">
                  ...
                  <p className="text-xs sm:text-sm text-gray-600">{invoiceData.deliveryNote}</p>
                </div>
              )}
              ...
              {invoiceData.terms && (
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-600">{invoiceData.terms}</p>
                </div>
              )}
            </div>
          )}
              ...
              <div className="flex flex-col">
                <div className="border border-gray-300 rounded-lg p-4 h-32 sm:h-40 bg-gray-50 flex items-end justify-end">
                  <div className="text-right">
                    <p className="text-sm sm:text-base font-semibold text-gray-900">{invoiceData.seller.name}</p>
                  </div>
                </div>
                <div className="mt-3 text-center">
