...
export default function CustomersPage() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await apiClient.get('/api/customers?limit=200');
      if (res.ok) setItems((await res.json()).customers || []);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="sticky top-0 z-20 border-b border-gray-200 bg-white shadow-sm">
        <div className="flex min-h-16 items-center justify-between gap-3 px-4">
          ...
          <p className="hidden max-w-3xl truncate text-sm text-gray-600 md:block">
            Manage buyer organizations and their receivable accounts.
          </p>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto p-3 sm:p-4 grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
        <form onSubmit={onSubmit} className="bg-white border border-gray-200 rounded-lg p-3 sm:p-4 space-y-3 lg:col-span-1 h-fit text-sm">
          <h2 className="font-semibold text-gray-900">{editingId ? 'Edit Customer' : 'New Customer'}</h2>
        ...
        <section className="bg-white border border-gray-200 rounded-lg p-3 sm:p-4 lg:col-span-2">
          <h2 className="font-semibold text-gray-900 mb-3">Customer List</h2>
          {loading ? <p className="text-gray-600">Loading...</p> : (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[640px] text-xs sm:text-sm">
                <thead><tr className="text-left border-b"><th className="py-2">Name</th><th>Code</th><th>GSTIN</th><th>Phone</th><th>Status</th><th className="text-right">Actions</th></tr></thead>
                <tbody>
                  {items.map((item) => (
                    <tr key={item.id} className="border-b">
                      <td className="py-2 font-medium">{item.name}</td><td>{item.code || '-'}</td><td>{item.gstin || '-'}</td><td>{item.phone || '-'}</td><td>{item.isActive ? 'Active' : 'Inactive'}</td>
                      <td className="text-right space-x-2"><button className="text-blue-600 text-xs sm:text-sm" onClick={() => { setEditingId(item.id); setForm({ name: item.name || '', code: item.code || '', gstin: item.gstin || '', phone: item.phone || '', city: item.city || '', state: item.state || '', openingDue: item.openingDue || '0', isActive: item.isActive }); }}>Edit</button><button className="text-red-600 text-xs sm:text-sm" onClick={() => onDelete(item.id)}>Delete</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
