...
export default function ProcessTypesPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(err.error || 'Failed to save process type');
        return;
      }
      setEditingId(null);
      setForm(emptyForm);
  ...
  const onDelete = async (id: number) => {
    if (!confirm('Delete this process type?')) return;
    const res = await apiClient.delete(`/api/process-types/${id}`);
    if (!res.ok) {
      const err = await res.json();
      alert(err.error || 'Failed to delete process type');
      return;
    }
    await fetchItems();
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 sticky top-0 z-20 shadow-sm">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="text-lg sm:text-xl font-bold text-gray-900">Process Types</h1>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto p-3 sm:p-4 grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
        <form onSubmit={onSubmit} className="bg-white border border-gray-200 rounded-lg p-3 sm:p-4 space-y-3 lg:col-span-1 h-fit text-sm">
          <h2 className="font-semibold text-gray-900">{editingId ? 'Edit Process Type' : 'New Process Type'}</h2>
        ...
        <section className="bg-white border border-gray-200 rounded-lg p-3 sm:p-4 lg:col-span-2">
          <h2 className="font-semibold text-gray-900 mb-3">Process Type List</h2>
