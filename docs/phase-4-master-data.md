...
1. Build UI pages for supplier/vendor/customer management. (Completed)
...
These components use search APIs with debounced query and keyboard navigation, and are ready for use in Purchase, Sales, and Payment forms.

## Note

Process types are now treated as mostly predefined operational configuration.
The dedicated Process Type master UI and CRUD APIs were removed, while runtime lookup via `GET /api/search/process-types` remains for process entry flows.
